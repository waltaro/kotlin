...
internal open class DefaultPlatformImplementations {
    ...
    companion object {
        @JvmField
        @InlineExposed
        val INSTANCE: DefaultPlatformImplementations = run {
            val version = getJavaVersion()
            try {
                if (version >= 0x10008)
                    return@run Class.forName("kotlin.jdk8.internal.JDK8PlatformImplementations").newInstance() as DefaultPlatformImplementations
            } catch (e: ClassNotFoundException) {}

            try {
                if (version >= 0x10007)
                    return@run Class.forName("kotlin.jdk7.internal.JDK7PlatformImplementations").newInstance() as DefaultPlatformImplementations
            } catch (e: ClassNotFoundException) {}

            DefaultPlatformImplementations()
        }
